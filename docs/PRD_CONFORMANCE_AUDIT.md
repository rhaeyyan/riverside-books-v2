...
`docs/IMPLEMENTATION_PLAN.md` and `docs/implementation_plan.md` are two
separate git-tracked files differing only by case, with genuinely different
committed content. On case-insensitive filesystems (macOS, Windows) they
collide onto one file on disk, which will confuse `git status`/`checkout`/
`pull` for anyone working locally. Worth a follow-up to rename one.
